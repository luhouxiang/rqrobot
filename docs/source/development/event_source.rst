...
了解事件，首先要从 rqrobot 的事件驱动说起。

rqrobot 大部分的组件是以 :code:`add_listener` 的方式进行事件的注册。举例来说:
...
程序化交易中很多需求，都可以通过注册事件的方式无缝插入到 rqrobot 中进行扩展。
...
.. image:: https://raw.githubusercontent.com/ricequant/rq-resource/master/rqrobot/progress_bar.png
...
rqrobot 整个回测模块是通过 :code:`rqrobot_mod_sys_simulation` 实现的，其中定义了基于Bar回测的 :code:`event_source` 和 :code:`simulation_broker`， 其中包含了 MarketEvent 和 OrderEvent 大部分事件源的定义和发布。
...
:code:`event` 函数是一个generator, 在 rqrobot_mod_sys_simulation 中主要返回 :code:`BEFORE_TRADING`, :code:`BAR`, :code:`AFTER_TRADING` 和 :code:`SETTLEMENT` 事件。rqrobot 在接受到对应的事件后，会自动的进行相应的 `publish_event` 操作，并且会自动 publish 相关的 `PRE_` 和 `POST_` 事件。
...
如果想查看详细的事件源相关的内容，建议直接阅读 `rqrobot_mod_sys_simulation` 源码，您会发现，扩展事件源比想象中要简单。

您也可以基于 `rqrobot_mod_sys_simulation` 扩展一个自定义的回测引擎，实现您特定的回测需求。
