...
class FuncatAPIMod(AbstractMod):
    def start_up(self, env, mod_config):
        ...
        from rqrobot.api import history_bars

        class rqrobotDataBackend(DataBackend):
            """
            目前仅支持日数据
            """
            ...
            def __init__(self):
                ...
                self.rqrobot_env = Environment.get_instance()

                self.rqrobot_env.event_bus.add_listener(EVENT.PRE_BEFORE_TRADING, self._pre_before_trading)
                self.rqrobot_env.event_bus.add_listener(EVENT.PRE_BAR, self._pre_handle_bar)

                self.fetch_data_by_api = True

            def _pre_before_trading(self, *args, **kwargs):
                calendar_date = self.rqrobot_env.calendar_dt.date()
            ...
            def _history_bars(self, order_book_id, bar_count, freq, dt):
                if self.fetch_data_by_api and ExecutionContext.phase() in (
                    ...
                    bars = self.rqrobot_env.data_proxy.history_bars(
            ...
            def get_start_date(self):
                """获取回溯开始时间
                """
                return str(self.rqrobot_env.config.base.start_date)
